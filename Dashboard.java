...
public class Dashboard extends javax.swing.JFrame {
    ...
    private ArrayList<Book> books = listOfBooks(); // Initialize books with the list of books
    private ArrayList<Book> borrowedBooks = new ArrayList<>();
    private ArrayList<String[]> borrowRecords = new ArrayList<>();
    List<Book> notAvailableBooks = getAllNotAvailableBooks();
...
public void addRowToTable(ArrayList<Book> filteredList) {
    ...
    for (int i = 0; i < filteredList.size(); i++) {
        rowData[0] = filteredList.get(i).getTittle();
...
public void setStaffForm(StaffForm staffForm) {
 ...
 int selectedRow = jTableBorrowed.getSelectedRow();
    if (selectedRow != -1) {
        ...
        // Add the returned book back to the availableBooks list
        for (Book book : books) {
            if (book.getTittle().equals(title) && book.getAuthor().equals(author)) {
        ...
        // Update the available table to reflect the changes
        addRowToTable();
        // Update the borrowed table to reflect the changes
